...
/// through [`write()`]'s `&str` signature would require a UTF-8 round-trip that
/// silently corrupts the bytes.
///
/// Policy is identical to [`write()`] — size cap, symlink rejection, canonical
/// `allowed_paths` glob enforcement, atomic rename — just with a `&[u8]`
/// payload.
pub fn write_bytes(
    path: &str,
    content: &[u8],

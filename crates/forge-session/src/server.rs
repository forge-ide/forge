...
pub async fn serve_with_session<P: Provider + 'static>(
    ...
    // functional. The alternative (hard-fail on load) would turn any stale
    // `.mcp.json` into a session-breaking error, which is worse than the
    // degraded-but-alive behavior here. Each server's health is tracked
    // per-server inside the manager; `list()` reflects the state so
    // `list_mcp_servers` (shell-side) can show the failure reason.
    //
    // This `McpManager` instance is the daemon-local one. A second
    // instance lives inside `forge-shell`'s Tauri app for the UI-facing
    // commands — the two read the same on-disk `.mcp.json`, so a `list()`
    // from either side reports the same configured set. Toggling a
    // server from the shell does not affect the daemon's already-running
    // session (toggle is a UI / next-session concern). See the F-132 PR
    // for the rationale on the split topology.

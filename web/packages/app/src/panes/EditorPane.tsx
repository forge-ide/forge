...
//
// The component is intentionally test-friendly: every side-channel
// (iframe URL, postMessage dispatch, IPC invoke, save debouncer) is
// injectable through `EditorPaneProps`. Vitest runs the component with
// a stub `target` iframe and stubbed helpers — Monaco itself is never
// mounted in jsdom (see monaco-host/README for the rationale).
 ...
 *  into `web/packages/monaco-host/dist/` — the actual wiring of that dist
 *  into the app bundle is a follow-up; this default is the target path. */
...
export interface EditorPaneProps {
   ...
   *  the real Tauri-invoking helpers. */
...
export const EditorPane: Component<EditorPaneProps> = (props) => {
  ...
  const sendOpen = async (): Promise<void> => {
    try {
      const file = await readFile(props.sessionId, props.workspaceRoot, props.path);
  ...
  const persist = async (value: string): Promise<void> => {
    try {
      await writeFile(props.sessionId, props.workspaceRoot, props.path, value);
  ...
  const handleMessage = (event: MessageEvent): void => {
    ...
    switch (data.kind) {
      ...
      // today; `client.message` / `client.notification` are LSP traffic that
      // F-123 will route to forge-lsp (out of scope here).
      default:
        break;
    }
  };

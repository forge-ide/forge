...
### 2.3 Provider
A backend that speaks a supported agent protocol. Anthropic and OpenAI are first-class built-ins; any OpenAI-compatible endpoint — including local model servers like Ollama, LM Studio, and vLLM — is configured as a `custom_openai` provider entry (the Add Provider modal ships local-Ollama and local-LM-Studio presets that auto-fill the endpoint and shape the auth field for each backend — Ollama defaults to API key `ollama` for fronting-proxy compat, LM Studio defaults to keyless and lets the user click "Test connection" to fetch the loaded model). Providers have accent colors (per DESIGN.md) and credentials. Multiple providers can be active within one session. Providers can be invoked directly without defining an agent — `forge run provider anthropic/sonnet-4.5` is a legitimate way to start work.
